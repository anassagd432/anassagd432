...
INP = os.path.abspath(sys.argv[1]) if len(sys.argv) > 1 else os.path.join(HERE, "..", "source-photo.jpg")
PREPPED = os.path.join(HERE, "..", "source-prepped.png")
OUT_SVG = os.path.join(HERE, "..", "anass-ascii.svg")

print(f"Step 1: Loading {INP} and isolating subject...")
...
prepped_img = Image.fromarray(out, mode="L")
prepped_img.save(PREPPED)
print(f"Saved prepped image to {PREPPED}")

print("Step 3: Generating ASCII SVG...")
COLS = 100
ROWS = 53
CELL_W = 8
CELL_H = 15
...
im = ImageEnhance.Brightness(prepped_img).enhance(BRIGHTNESS)
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">anass@github: ~$ ./portrait.sh</text>')
...
with open(OUT_SVG, "w") as f:
    f.write(svg)
print(f"Done! Wrote {OUT_SVG} ({len(svg)} bytes, {CANVAS_W}x{CANVAS_H})")
