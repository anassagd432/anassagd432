#!/usr/bin/env python3
"""
Render "AGDI" (or custom text) as an EXTRUDED 3D wordmark rasterized to ASCII,
and emit it as an SVG that animates on GitHub (SMIL only).
"""
import argparse
import html
import math
...
TEXT = os.environ.get("WORDMARK_TEXT", "AGDI")
MASK_H = 300
TRACKING = 0.14
LINE_GAP = 1.20
DEPTH_FRAC = 0.34
...
def emit(frames, mode, out, dur, reveal):
    ...
    p = [
        ...
        f'<rect x="0.5" y="0.5" width="{canvas_w-1:.0f}" height="{canvas_h-1:.0f}" rx="12" fill="none" stroke="{FRAME}" stroke-width="1"/>',
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" font-size="11.5" text-anchor="middle">anass@github: ~$ ./wordmark.sh --3d</text>')
    ...
    def frame_g(rows, extra=""):
        ...
        for ry, line in enumerate(rows):
            ...
            out_rows.append(
                f'<text xml:space="preserve" x="{x:.1f}" y="{y:.1f}" font-size="{fs:.1f}" textLength="{len(body)*CELL_W:.1f}" lengthAdjust="spacing">{html.escape(body)}</text>'
            )
    ...
    p.append(f'<clipPath id="wipe"><rect x="{PAD}" y="{art_top:.1f}" height="{art_h:.1f}" width="0">'
             f'<animate attributeName="width" from="0" to="{art_w:.0f}" begin="0s" dur="{reveal:.2f}s" fill="freeze"/></rect></clipPath>')
    p.append(f'<g clip-path="url(#wipe)">{frame_g(frames[0])}'
             f'<set attributeName="opacity" to="0" begin="{reveal:.2f}s"/></g>')
    p.append(f'<rect x="{PAD}" y="{art_top+2:.1f}" width="{CELL_W*1.6:.1f}" height="{art_h-4:.1f}" fill="{INK}" opacity="0.16">'
